...
export interface GradientWavesProps {
  ...
  height?: number;
  /** Distance over which waves fade into haze and transparency. */
  fogDepth?: number;
  /** Raymarch quality tier. Each step is a full-screen shader iteration. */
  detail?: Detail;
  brightness?: number;
  opacity?: number;
  ...
  /** Device-pixel-ratio cap. Lower is cheaper on retina displays. */
  maxDpr?: number;
  className?: string;
}
...
export default function GradientWaves({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uEnableMouse: { value: mouseInteraction && !reduced },
    ...
    // Props are pushed to uniforms by the effect below rather than rebuilding
    // the GL context; only maxDpr changes the renderer itself.
  ...
  useEffect(() => {
    ...
    write(u.uHorizonColor.value as Float32Array, horizonColor);
    write(u.uWaveColor.value as Float32Array, waveColor);
    write(u.uCrestColor.value as Float32Array, crestColor);
  }, [
    horizonColor,
    waveColor,
    crestColor,
